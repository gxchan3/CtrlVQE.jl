...
"""
struct CVAREnergy{F} <: CostFunctions.EnergyFunction{F}
...
Base.length(fn::CVAREnergy) = Parameters.count(fn.device)
...
function CostFunctions.cost_function(fn::CVAREnergy; callback=nothing)
...
function CostFunctions.grad_function_inplace(fn::CVAREnergy{F}; ϕ=nothing) where {F}
